(function () {
  ...
  config.events.forEach((event) => {
    ...
    const cardTitle = event.cardLabel || event.title;
    const ctaTitle = /invite/i.test(cardTitle) ? cardTitle : `${cardTitle} Invite`;
    ...
    card.href = href;
    card.setAttribute("aria-label", `Open ${ctaTitle}`);

    image.src = event.cardImage;
    image.alt = event.cardAlt || ctaTitle;

    label.className = `invite-label ${event.labelClass || ""}`.trim();
    hint.className = "invite-label__hint";
    hint.textContent = "Tap to open";
    title.className = "invite-label__title";
    title.textContent = ctaTitle;
    arrow.className = "invite-label__arrow";
    arrow.setAttribute("aria-hidden", "true");
    arrow.textContent = ">";

    label.append(hint, title, arrow);
